...
def test_image_model_test_reports_reference_image_capability_separately(tmp_path):
    """Image model test should not fail text-to-image when image input is unsupported."""
    test_image = tmp_path / 'test.png'
    test_image.write_bytes(b'not-used-by-fake-service')
    calls = []
    ...
    class FakeAIService:
        def generate_image(self, **kwargs):
            ...
            return SimpleNamespace(size=(320, 180))

    with (
        patch.object(settings_controller, 'AIService', return_value=FakeAIService()),
        patch.object(settings_controller, '_get_test_image_path', return_value=test_image),
        patch.object(
            settings_controller.Settings,
            'get_settings',
            return_value=SimpleNamespace(image_aspect_ratio='16:9', image_resolution='2K'),
        ),
    ):
        result_data, message = settings_controller._test_image_model()

    assert result_data['text_to_image'] is True
    assert result_data['reference_image'] is False
    assert '不支持请求中的能力' in result_data['reference_image_error']
    assert '文生图测试成功' in message
    assert calls[0].get('ref_image_path') is None
    assert calls[1].get('ref_image_path') == str(test_image)

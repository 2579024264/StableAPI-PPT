...
class PageImageVersion(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'image_url': self.image_path if self.image_path and (
                self.image_path.startswith('local-file://') or self.image_path.startswith('local-result://')
            ) else (
                f'/files/{project_id}/pages/{self.image_path.split("/")[-1]}' if self.image_path and project_id else None
            ),

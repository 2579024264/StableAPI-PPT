...
def _parse_local_export_page_ids() -> list[str]:
    raw = request.form.get('page_ids', '')
    return [item.strip() for item in raw.split(',') if item.strip()]


def _collect_local_export_images(tmpdir: str, expected_page_ids: list[str] | None = None):
    image_items = []
    files = request.files.getlist('images')
    if not files:
        return [], "No images provided"

    expected = expected_page_ids or []
    for index, file in enumerate(files):
        page_id = request.form.get(f'page_ids[{index}]')
        if not page_id and index < len(expected):
            page_id = expected[index]
        if not page_id:
            page_id = f"page_{index + 1}"

        filename = secure_filename(file.filename or f"slide_{index + 1}.png")
        if not filename:
        ...
        path = os.path.join(tmpdir, f"{index:04d}_{filename}")
        file.save(path)
        image_items.append({
            "page_id": page_id,
            "order": index,
...
def _send_bytes(data: bytes, mimetype: str, filename: str):
    return send_file(
        io.BytesIO(data),
        mimetype=mimetype,
        as_attachment=True,
...
@export_bp.route('/<project_id>/export/local/pptx', methods=['POST'])
def export_local_pptx(project_id):
    """Export PPTX from browser-local images uploaded for this request only."""
    try:
        project = db.session.get(Project, project_id)
        if not project:
            return not_found('Project')

        filename = secure_filename(request.form.get('filename') or f'presentation_{project_id}.pptx')
        if not filename:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            image_items, error = _collect_local_export_images(tmpdir, _parse_local_export_page_ids())
            if error:
            ...
            pptx_bytes = ExportService.create_pptx_from_images(
                [item["path"] for item in image_items],
                output_file=None,
        ...
        logger.exception("Failed to export local PPTX")
        return error_response('SERVER_ERROR', str(e), 500)


@export_bp.route('/<project_id>/export/local/pdf', methods=['POST'])
def export_local_pdf(project_id):
    """Export PDF from browser-local images uploaded for this request only."""
    try:
        project = db.session.get(Project, project_id)
        if not project:
            return not_found('Project')

        filename = secure_filename(request.form.get('filename') or f'presentation_{project_id}.pdf')
        if not filename:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            return _send_bytes(pdf_bytes, 'application/pdf', filename)

    except Exception as e:
        logger.exception("Failed to export local PDF")
        return error_response('SERVER_ERROR', str(e), 500)


@export_bp.route('/<project_id>/export/local/images', methods=['POST'])
def export_local_images(project_id):
    """Export raw images from browser-local uploads without persisting server-side."""
    try:
        project = db.session.get(Project, project_id)
        if not project:
            return not_found('Project')
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if len(image_items) == 1:
                ...
                with open(item["path"], 'rb') as f:
                    data = f.read()
                filename = f'slide_{item["page_id"]}_{timestamp}{item["ext"]}'
                mimetype = 'image/jpeg' if item["ext"] in {'.jpg', '.jpeg'} else f'image/{item["ext"].lstrip(".")}'
                return _send_bytes(data, mimetype, filename)

            zip_buffer = io.BytesIO()
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
                for item in image_items:
                    zf.write(item["path"], f'slide_{item["order"] + 1:03d}{item["ext"]}')
            zip_buffer.seek(0)
            return _send_bytes(
                zip_buffer.getvalue(),
                'application/zip',
                f'slides_{project_id}_{timestamp}.zip',
            )

    except Exception as e:
        logger.exception("Failed to export local images")
        return error_response('SERVER_ERROR', str(e), 500)
...
@export_bp.route('/<project_id>/export/local/editable-pptx', methods=['POST'])
def export_local_editable_pptx(project_id):
    """Create an async editable PPTX export from browser-local images."""
    temp_dir = None
    try:
        project = db.session.get(Project, project_id)
        if not project:
        ...
        selected_page_ids = _parse_local_export_page_ids()
        pages = get_filtered_pages(project_id, selected_page_ids if selected_page_ids else None)
        if not pages:
            return bad_request("No pages found for project")

        filename = secure_filename(request.form.get('filename') or f'presentation_editable_{project_id}.pptx')
        if not filename:
        ...
        try:
            max_depth = _form_int('max_depth', 1)
            max_workers = _form_int('max_workers', 4)
        except (TypeError, ValueError):
            return bad_request("max_depth and max_workers must be integers")
        ...
        temp_dir = tempfile.mkdtemp(prefix=f"local_editable_{task.id}_")
        image_items, image_error = _collect_local_export_images(temp_dir, selected_page_ids)
        if image_error:
            shutil.rmtree(temp_dir, ignore_errors=True)
            db.session.delete(task)
            db.session.commit()
            return bad_request(image_error)
        ...
        task_manager.submit_task(
            ...
            filename=filename,
            file_service=file_service,
            page_ids=[item["page_id"] for item in image_items],
            max_depth=max_depth,
            max_workers=max_workers,
            export_extractor_method=export_extractor_method,
            export_inpaint_method=export_inpaint_method,
            enable_icon_subject_extraction=enable_icon_subject_extraction,
            local_image_items=image_items,
        ...
        return success_response(
            ...
            message="Local editable PPTX export task created"
        )

    except Exception as e:
        if temp_dir:
            shutil.rmtree(temp_dir, ignore_errors=True)
        logger.exception("Error creating local editable PPTX export task")
        return error_response('SERVER_ERROR', str(e), 500)


@export_bp.route('/<project_id>/export/local/video', methods=['POST'])
def export_local_video(project_id):
    """Create an async narration video export from browser-local images."""
    temp_dir = None
    try:
        project = db.session.get(Project, project_id)
        if not project:
        ...
        filename = secure_filename(request.form.get('filename') or f'narration_{project_id}.mp4')
        if not filename:
        ...
        voice = request.form.get('voice') or current_app.config.get('TTS_DEFAULT_VOICE_ZH', 'zh-CN-XiaoxiaoNeural')
        if not request.form.get('voice'):
            from services.tts_video_service import get_default_voice
            voice = get_default_voice(language, dict(current_app.config))

        rate = request.form.get('rate') or current_app.config.get('TTS_DEFAULT_RATE', '+0%')
        try:
            speed = max(0.7, min(_form_float('speed', 1.0), 1.2))
        except (TypeError, ValueError):
            speed = 1.0

        generate_narration = _form_bool('generate_narration', True)
        enable_ken_burns = _form_bool('enable_ken_burns', False)
        include_no_image_pages = _form_bool('include_no_image_pages', False)
        presentation_topic = request.form.get('presentation_topic') or project.idea_prompt or ''
        narration_config_raw = request.form.get('narration_config')
        try:
            narration_config_input = json.loads(narration_config_raw) if narration_config_raw else None
        except json.JSONDecodeError:
            return bad_request("narration_config must be valid JSON")
        narration_config = normalize_narration_generation_config(
            narration_config_input,
        ...
        temp_dir = tempfile.mkdtemp(prefix=f"local_video_{task.id}_")
        image_items, image_error = _collect_local_export_images(temp_dir, selected_page_ids)
        if image_error and not include_no_image_pages:
            shutil.rmtree(temp_dir, ignore_errors=True)
            db.session.delete(task)
            db.session.commit()
            return bad_request(image_error)
        ...
        has_any_image = bool(image_items)
        if not has_any_image and not include_no_image_pages:
            shutil.rmtree(temp_dir, ignore_errors=True)
            db.session.delete(task)
            db.session.commit()
        ...
        return success_response(
            ...
            message="Local video export task created"
        )

    except Exception as e:
        if temp_dir:
            shutil.rmtree(temp_dir, ignore_errors=True)
        logger.exception("Error creating local video export task")
        return error_response('SERVER_ERROR', str(e), 500)

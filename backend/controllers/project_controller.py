...
def _sse_error_message(error: Exception) -> str:
    """Return a user-facing SSE error without leaking obvious credentials."""
    ...
    raw = re.sub(
        r'(?i)(api[_-]?key|authorization|bearer|token|access[_-]?key|secret[_-]?key)'
        r'([=:]\s*|\s+)[^\s,;}\]]+',
        r'\1\2[redacted]',
        raw,
    )
    ...
    if 'Failed to parse response as JSON' in raw and ': PING' in raw:
        return (
            '生成失败：模型接口返回了 SSE 心跳而不是 Gemini JSON 响应。'
            '请检查“AI 提供商格式”是否选错；使用 stableapi.io、AIHubMix 等 OpenAI 兼容代理时，'
            '通常应选择 OpenAI 格式，而不是 Gemini。'
        )

    return f'生成失败：{raw}'


@project_bp.route('/<project_id>/local-results/<result_id>', methods=['GET'])
def get_local_result(project_id, result_id):
    """
    GET /api/projects/{project_id}/local-results/{result_id}

    Returns a temporary generated file for strict browser-local file mode.
    The browser is expected to persist the response into IndexedDB/OPFS.
    """
    result = local_result_store.get(result_id)
    if not result or result.project_id != project_id:
        return not_found('Local result')
...
def _strict_local_files_requested(data: dict | None = None) -> bool:
    """Whether this request should avoid persisting user file bytes server-side."""
    header = (request.headers.get('X-Strict-Local-Files') or '').lower()
    ...
    def sse_generate():
        with app.app_context():
            try:
                ...
                yield _sse_event('error', {'message': _sse_error_message(e)})

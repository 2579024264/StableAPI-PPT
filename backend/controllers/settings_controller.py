...
def _settings_api_credentials(settings, provider):
    """Resolve API credentials from saved settings and environment defaults."""
    provider = (provider or "openai").lower()
    if provider == "volcengine":
        return (
            settings.api_key if settings.api_key is not None else Config.VOLCENGINE_API_KEY,
            settings.api_base_url if settings.api_base_url is not None else Config.VOLCENGINE_API_BASE,
        )
    if provider == "gemini":
        return (
            settings.api_key if settings.api_key is not None else Config.GOOGLE_API_KEY,
            settings.api_base_url if settings.api_base_url is not None else Config.GOOGLE_API_BASE,
        )
    return (
...
@settings_bp.route("/models", methods=["GET"], strict_slashes=False)
def list_provider_models():
    """
    GET /api/settings/models - List models available to the current API key.

    This is primarily used for NewAPI/OpenAI-compatible gateways. The gateway
    filters /v1/models by the token's group permissions, so the frontend can
    present the exact model list available to the user.
    """
    try:
        settings = Settings.get_settings()
        ...
        try:
            ...
            detail = detail_payload.get("error", {}).get("message") or detail_payload.get("message") or ""
        except Exception:
            detail = e.response.text[:200] if e.response is not None else ""
        return error_response(
            "LIST_MODELS_ERROR",
            f"拉取模型列表失败（HTTP {status}）{(': ' + detail) if detail else ''}",
            502,
        )
    except Exception as e:
...
def _test_image_model():
    ...
    # First verify the core text-to-image capability. Some OpenAI-compatible
    # proxy routes support image generation but reject image-input/editing
    # requests; testing with a reference image first makes that failure look
    # like the whole model is unavailable.
    ...
    try:
        ref_result = ai_service.generate_image(
            prompt=prompt,
        ...
        if ref_result is None:
            result_data["reference_image_error"] = "参考图测试返回空结果"
            return result_data, "图像生成模型测试成功；但参考图/模板图能力未通过"
        result_data["reference_image"] = True
        result_data["reference_image_size"] = ref_result.size
        return result_data, "图像生成模型测试成功"
    except Exception as e:
        result_data["reference_image_error"] = str(e)
        return result_data, "图像生成模型文生图测试成功；但当前渠道不支持参考图/模板图能力"

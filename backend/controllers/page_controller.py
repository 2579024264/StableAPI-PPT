...
def _strict_local_files_requested(data=None) -> bool:
...
@page_bp.route('/<project_id>/pages/<page_id>/local-image', methods=['POST'])
def confirm_local_page_image(project_id, page_id):
    """
    POST /api/projects/{project_id}/pages/{page_id}/local-image

    Strict local file mode callback. The browser has persisted a generated image
    locally and sends back its stable local-file:// URL for project metadata.
    """
    try:
        page = Page.query.get(page_id)
        if not page or page.project_id != project_id:
        ...
        local_file_url = data.get('local_file_url')
        if not isinstance(local_file_url, str) or not local_file_url.startswith('local-file://'):
            return bad_request("local_file_url must start with local-file://")
        ...
        if current_version:
            current_version.image_path = local_file_url

        page.generated_image_path = local_file_url
        ...
        db.session.rollback()
        logger.error(f"Failed to confirm local image for page {page_id}: {e}")
        return error_response('SERVER_ERROR', 'An internal server error occurred', 500)

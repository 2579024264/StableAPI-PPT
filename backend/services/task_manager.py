...
def save_image_with_version(image, project_id: str, page_id: str, file_service,
                            ...
                            strict_local_results: bool = False) -> tuple[str, int]:
    """
    保存图片并创建历史版本记录的公共函数
    ...
    if strict_local_results:
        result = local_result_store.put_image(
            image,
        ...
        image_path = f"local-result://{result.id}"
...
def export_editable_pptx_with_recursive_analysis_task(
    ...
    Args:
        ...
        app: Flask应用实例
    """
    logger.info(f"🚀 Task {task_id} started: export_editable_pptx_with_recursive_analysis (project={project_id}, depth={max_depth}, workers={max_workers}, extractor={export_extractor_method}, inpaint={export_inpaint_method}, icon_subject_extraction={enable_icon_subject_extraction}, strict_local_export_result={strict_local_export_result})")
    ...
    with app.app_context():
        ...
        try:
            ...
            image_paths = []
            if local_image_items is not None:
                for item in local_image_items:
                    img_path = item.get('path') if isinstance(item, dict) else None
            ...
            if strict_local_export_result:
                output_dir = local_temp_dir or tempfile.mkdtemp(prefix=f"local_editable_output_{task_id}_")
            else:
                output_dir = os.path.join(app.config['UPLOAD_FOLDER'], project_id, 'exports')
                os.makedirs(output_dir, exist_ok=True)

            output_path = os.path.join(output_dir, filename)
            if not strict_local_export_result and os.path.exists(output_path):
            ...
            editability_upload_folder = None
            if strict_local_export_result:
                editability_upload_folder = os.path.join(output_dir, 'editability_work')
                os.makedirs(editability_upload_folder, exist_ok=True)
            ...
            _, export_warnings = ExportService.create_editable_pptx_with_recursive_analysis(
                ...
                fail_fast=fail_fast,
                upload_folder=editability_upload_folder,
            )
            ...
            if strict_local_export_result:
                with open(output_path, 'rb') as f:
                    result = local_result_store.put_bytes(
                        f.read(),
                        project_id=project_id,
                        filename=filename,
                ...
                download_path = f"/api/projects/{project_id}/local-results/{result.id}"
            else:
            ...
            if task:
                ...
                task.set_progress({
                    ...
                    "local_result_id": local_result_id,
                    "strict_local_export_result": strict_local_export_result,
                    "filename": filename,
            ...
            if strict_local_export_result and local_temp_dir and os.path.exists(local_temp_dir):
        ...
        try:
            ...
            if local_image_items is not None:
                for item in local_image_items:
                    ...
                    if page_id and img_path and os.path.exists(img_path):
                        local_image_by_page_id[str(page_id)] = img_path
            ...
            if include_no_image_pages:
                ...
                if strict_local_export_result:
                    placeholder_root = local_temp_dir or tempfile.mkdtemp(prefix=f"local_video_placeholder_{task_id}_")
                    placeholder_dir = os.path.join(placeholder_root, f'_placeholder_{task_id}')
                else:
            ...
            for page in pages:
                page_id = str(page.id)
                if page_id in local_image_by_page_id:
                    valid_pages.append((page, local_image_by_page_id[page_id]))
                    continue

                if not strict_local_export_result and page.generated_image_path:
                    img_path = file_service.get_absolute_path(page.generated_image_path)
                    if os.path.exists(img_path):
            ...
            if strict_local_export_result:
                output_dir = local_temp_dir or tempfile.mkdtemp(prefix=f"local_video_output_{task_id}_")
            else:
                output_dir = os.path.join(app.config['UPLOAD_FOLDER'], project_id, 'exports')
                os.makedirs(output_dir, exist_ok=True)

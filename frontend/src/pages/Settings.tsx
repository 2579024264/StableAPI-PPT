...
import { Home, Key, Image, Zap, Save, RotateCcw, Globe, FileText, Brain, ArrowUp, HelpCircle, Link2, ChevronDown, Volume2, Info, RefreshCw, CheckCircle, Scale } from 'lucide-react';
  ...
  zh: {
    ...
    settings: {
      ...
      fields: {
        ...
        textModel: "文本大模型", textModelPlaceholder: "留空使用环境变量配置 (如: gemini-3.1-pro-preview)",
        ...
        imageCaptionModel: "图片识别模型", imageCaptionModelPlaceholder: "留空使用环境变量配置 (如: gemini-3.1-pro-preview)",
        imageCaptionModelDesc: "用于识别参考文件中的图片并生成描述",
        modelListRefresh: "刷新模型列表",
        modelListLoading: "正在拉取模型列表...",
        modelListLoaded: "已加载 {{count}} 个可用模型",
        modelListEmpty: "尚未拉取到模型列表，可手动输入模型名",
        modelListError: "模型列表拉取失败：{{message}}",
      ...
      messages: {
        ...
        urlKeySaved: "已从链接保存 API Key，并应用默认模型",
        urlKeySaveFailed: "链接中的 API Key 保存失败",
  ...
  en: {
    ...
    settings: {
      ...
      fields: {
        ...
        textModel: "Text Model", textModelPlaceholder: "Leave empty to use env config (e.g., gemini-3.1-pro-preview)",
        ...
        imageCaptionModel: "Image Caption Model", imageCaptionModelPlaceholder: "Leave empty to use env config (e.g., gemini-3.1-pro-preview)",
        imageCaptionModelDesc: "Model for recognizing images in reference files and generating descriptions",
        modelListRefresh: "Refresh model list",
        modelListLoading: "Loading model list...",
        modelListLoaded: "{{count}} available models loaded",
        modelListEmpty: "No model list loaded yet; you can still enter a model name manually",
        modelListError: "Failed to load model list: {{message}}",
      ...
      messages: {
        ...
        urlKeySaved: "API Key from link saved and default models applied",
        urlKeySaveFailed: "Failed to save API Key from link",
        ...
        <div className="flex flex-col gap-3 text-sm text-gray-600 dark:text-foreground-tertiary sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            ...
            <a
              ...
              className="inline-flex items-center gap-1 text-banana-700 dark:text-banana-300 hover:underline"
            >
              <Scale size={14} />
              <span>{t('settings.about.license')}: AGPL-3.0</span>
            </a>
            {updateInfo && (
              <div className={updateInfo.update_available ? 'text-banana-700 dark:text-banana-300' : 'text-gray-600 dark:text-foreground-tertiary'}>
              ...
              <div className="flex flex-col items-center gap-3 py-2 text-center">
                ...
                {updateInfo.update_available && (
                  <ArrowUp
                    ...
                    className="text-banana-600 dark:text-banana-300"
                    aria-hidden="true"
                  />
                )}
                <p className={updateInfo.update_available
                  ? 'text-xl font-semibold text-banana-700 dark:text-banana-300'
...
export const Settings: React.FC = () => {
  ...
  const availableModelOptions = Array.from(new Set([...availableModels, ...DEFAULT_MODEL_OPTIONS]))
  ...
  const loadAvailableModels = async (override?: { provider?: string; api_key?: string; api_base_url?: string }) => {
      ...
      if (response.data) {
        ...
        if (inboundApiKey) {
          ...
          try {
            ...
            show({
              message: `${t('settings.messages.urlKeySaveFailed')}: ${saveError?.response?.data?.error?.message || saveError?.message || t('settings.messages.unknownError')}`,
              type: 'error'
            });
          }
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (otherData.ai_provider_format === 'openai' && !otherData.api_base_url) {
        payload.api_base_url = DEFAULT_API_BASE_URL;
      }
      ...
      if (response.data) {
        ...
        if (api_key) {
          localStorage.setItem(URL_API_KEY_LOCAL_STORAGE_KEY, api_key);
        }
      ...
      async () => {
        ...
        try {
          ...
          if (response.data) {
            ...
            localStorage.removeItem(URL_API_KEY_LOCAL_STORAGE_KEY);
            setAvailableModels([]);
            setModelsError('');
            ...
            {field.options.map((option) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  value === option.value
                    ? 'bg-gradient-to-r from-[#AD73F6] to-[#7B83FF] text-white shadow-md shadow-banana-500/20'
      ...
      <div key={item.modelKey} className="pb-6 border-b border-gray-200 dark:border-border-primary last:border-b-0 last:pb-0 space-y-3">
        ...
        <div>
          ...
          >
            ...
            {Array.from(new Set([
              formData[item.modelKey] as string,
              ...availableModelOptions,
            ].filter(Boolean))).map((model) => (
              <option key={`${String(item.modelKey)}-${model}`} value={model}>
                {model}
              </option>
            ))}
          </select>
      ...
      <div className="space-y-8">
        ...
        <div data-testid="global-api-config-section">
          ...
          {usesVolcengineCampaignPromo ? (
            <div className="mt-3 pl-4 border-l-4 border-[#AD73F6] dark:border-[#A599FF]">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-700 dark:text-foreground-secondary">
                    ...
                    <a href={volcengineAgentPlansUrl} target="_blank" rel="noopener noreferrer" className="text-banana-700 hover:text-banana-800 dark:text-banana-300 dark:hover:text-banana-200 underline font-medium">
                  ...
                  <div className="pt-1">
                    <p className="text-sm font-medium text-gray-800 dark:text-foreground-primary flex items-center gap-1.5">
                      <HelpCircle size={15} className="text-banana-600" />
                      ...
                      <li>
                        ...
                        <span className="inline-flex items-center gap-2">
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-banana-700 hover:text-banana-800 dark:text-banana-300 dark:hover:text-banana-200 underline font-medium"
                          >
                            {t(`${activeVolcenginePromoKey}.guideLink`)}
                          </a>
                          <button
                            onClick={() => copyToClipboard(volcengineAgentPlansUrl)}
                            className="text-xs px-2 py-0.5 rounded transition-colors bg-banana-100 hover:bg-banana-200 dark:bg-banana-900/30 dark:hover:bg-banana-900/50 text-banana-800 dark:text-banana-300"
                          >
        ...
        <div>
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-gray-900 dark:text-foreground-primary flex items-center">
                <FileText size={20} />
                <span className="ml-2">{t('settings.sections.modelConfig')}</span>
              </h2>
              <p className={`mt-1 text-sm ${modelsError ? 'text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-foreground-tertiary'}`}>
                {isLoadingModels
                  ? t('settings.fields.modelListLoading')
                  : modelsError
                    ? t('settings.fields.modelListError', { message: modelsError })
                    : availableModels.length > 0
                      ? t('settings.fields.modelListLoaded', { count: availableModels.length })
                      : t('settings.fields.modelListEmpty')}
              </p>
            </div>
            <Button
              variant="secondary"
            ...
            >
              {t('settings.fields.modelListRefresh')}
            </Button>
          </div>
          <div className="space-y-4">
                  ...
                  {!settings?.openai_oauth_connected && (
                    <div className="mt-3">
                      <button
                        ...
                        className="text-xs text-banana-700 dark:text-banana-300 hover:underline"
                      >
                        {t('settings.openaiOAuth.manualCallbackLabel')}
                      </button>
                      {manualCallbackOpen && (
                        <div className="mt-2 p-3 bg-[#F0EDFF] dark:bg-banana-900/30 border border-banana-200 dark:border-banana-700/40 rounded-lg">
                          <p className="text-xs text-banana-700 dark:text-banana-300 mb-2">{t('settings.openaiOAuth.manualCallbackHint')}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="pl-4 border-l-4 border-banana-300 dark:border-banana-600">
            <p className="flex items-start gap-1.5 text-sm text-gray-700 dark:text-foreground-secondary">
              <Info size={15} className="mt-0.5 flex-shrink-0 text-banana-600 dark:text-banana-300" />
              <span>{t('settings.serviceTest.tip')}</span>
            </p>
          </div>
          <div className="space-y-4">
            {[
...
export const SettingsPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(135deg,#FBF6FF_0%,#F5F4FF_46%,#F1F6FF_100%)] dark:bg-[linear-gradient(135deg,#17172D_0%,#15172B_54%,#101225_100%)]">
      ...
      {showTop && (
        <button
          ...
          className="fixed bottom-6 right-6 p-3 rounded-full bg-gradient-to-r from-[#AD73F6] to-[#7B83FF] text-white shadow-lg shadow-banana-500/20 hover:brightness-105 transition-all z-50"
        >
          <ArrowUp size={20} />
        </button>
      )}

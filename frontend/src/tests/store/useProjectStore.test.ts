...
describe('useProjectStore', () => {
  ...
  describe('流式大纲', () => {
    it('should reject when SSE returns an error event', async () => {
      const { result } = renderHook(() => useProjectStore())
      const message = '生成失败：status_code=400, 当前模型或商家不支持请求中的能力'

      vi.mocked(api.generateOutlineStream).mockImplementation(async (_projectId, callbacks) => {
      ...
      await act(async () => {
        await expect(result.current.generateOutlineStream()).rejects.toThrow(message)
      })

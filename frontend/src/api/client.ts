...
import { isLocalFileUrl, resolveLocalFileObjectUrl } from '@/services/localFileUrls';
import { isLocalResultUrl, strictLocalFilesEnabled } from '@/services/strictLocalFiles';
...
apiClient.interceptors.request.use(
  (config) => {
    ...
    if (strictLocalFilesEnabled && config.headers) {
      config.headers['X-Strict-Local-Files'] = 'true';
    }
...
export const getImageUrl = (path?: string, timestamp?: string | number): string => {
  ...
  if (isLocalFileUrl(path)) {
    return resolveLocalFileObjectUrl(path) || '';
  }
  if (isLocalResultUrl(path)) {
    return '';
  }

...
export const getLocalResultBlob = async (
  projectId: string,
  resultId: string,
): Promise<Blob> => {
  const response = await apiClient.get<Blob>(
    `/api/projects/${projectId}/local-results/${resultId}`,
    { responseType: 'blob' },
  );
  return response.data;
};

export const downloadLocalResult = async (
  projectId: string,
  resultId: string,
  filename?: string,
): Promise<void> => {
  const blob = await getLocalResultBlob(projectId, resultId);
  downloadBlob(blob, filename || 'export');
};

export const confirmLocalPageImage = async (
  projectId: string,
  pageId: string,
  localFileUrl: string,
): Promise<ApiResponse<Page>> => {
  const response = await apiClient.post<ApiResponse<Page>>(
    `/api/projects/${projectId}/pages/${pageId}/local-image`,
...
export interface LocalExportImageInput {
  pageId: string;
  file: File;
}
...
export const exportLocalPPTX = async (
  projectId: string,
  images: LocalExportImageInput[],
  options?: {
    filename?: string;
...
export const exportLocalPDF = async (
  projectId: string,
  images: LocalExportImageInput[],
  filename?: string,
): Promise<ApiResponse<{ download_url: string }>> => {
...
export const exportLocalImages = async (
  projectId: string,
  images: LocalExportImageInput[],
): Promise<ApiResponse<{ download_url: string }>> => {
  const formData = new FormData();
...
export const exportLocalEditablePPTX = async (
  projectId: string,
  images: LocalExportImageInput[],
  options?: {
    filename?: string;
...
export const exportLocalVideo = async (
  projectId: string,
  images: LocalExportImageInput[],
  options?: ExportVideoOptions,
): Promise<ApiResponse<{ task_id: string }>> => {
...
export const listMaterials = async (
  ...
  if (strictLocalFilesEnabled) {
    return { success: true, data: await listLocalMaterials(projectId) };
  }
...
export const uploadMaterial = async (
  ...
  if (strictLocalFilesEnabled) {
    const material = await uploadLocalMaterial(file, projectId, null);
    return {
      success: true,
      data: { ...material, caption: material.caption ?? undefined },
    };
  }
...
export const getMaterialByUrl = async (url: string): Promise<ApiResponse<Material>> => {
  if (strictLocalFilesEnabled) {
    const material = await getLocalMaterialByUrl(url);
    if (!material) {
...
const STABLEAPI_MODELS_URL = 'https://stableapi.io/v1/models';
...
export const getAvailableModels = async (params?: {
  ...
  try {
    const response = await apiClient.get<ApiResponse<{ models: string[] }>>('/api/settings/models', { params });
    return response.data;
  } catch (error: any) {
    const shouldFallbackToStableApi =
      error?.response?.status === 404 ||
      error?.code === 'ERR_NETWORK' ||
      !error?.response;
    if (!shouldFallbackToStableApi) {
      throw error;
    }

    const apiKey = params?.api_key || localStorage.getItem(URL_API_KEY_LOCAL_STORAGE_KEY) || '';
    if (!apiKey) {
      throw error;
    }

    const response = await apiClient.get(STABLEAPI_MODELS_URL, {
      headers: { Authorization: `Bearer ${apiKey}` },
    });
    return {
      success: true,
      data: { models: normalizeModelList(response.data) },
    };
  }
};

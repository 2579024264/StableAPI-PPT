...
const claimStrictLocalImageResults = async (project: Project): Promise<Project> => {
  if (!strictLocalFilesEnabled || !project.id) return project;

  const pages = [...(project.pages || [])];
  let changed = false;

  for (const page of pages) {
    const resultUrl = page.generated_image_path || page.generated_image_url;
    if (!page.id || !isLocalResultUrl(resultUrl)) continue;
    ...
    try {
      const blob = await api.getLocalResultBlob(project.id, resultId);
...
const debouncedUpdatePage = debounce(
  ...
  initializeProject: async (type, content, templateImage, templateStyle, referenceFileIds, aspectRatio) => {
    ...
    try {
      ...
      // 3. 如果有模板图片，上传模板
      let localTemplateImageUrl: string | undefined;
      if (templateImage) {
        try {
          const templateResponse = await api.uploadTemplate(projectId, templateImage);
          localTemplateImageUrl = templateResponse.data?.template_image_url;
        } catch (error) {
      ...
      if (project) {
        if (localTemplateImageUrl) {
          project.template_image_url = localTemplateImageUrl;
      ...
      try {
        ...
        if (task.status === 'COMPLETED') {
          ...
          if (task.task_type === 'EXPORT_EDITABLE_PPTX' && task.progress) {
            ...
            const localResultId = progress?.local_result_id;
            if (strictLocalFilesEnabled && progress?.strict_local_export_result && localResultId) {
              devLog('[导出可编辑PPTX] 领取本地临时导出结果:', localResultId);
              const filename = progress?.filename || 'presentation_editable.pptx';
              const blob = await api.getLocalResultBlob(projectId, localResultId);
              const stored = await storeLocalExportBlob(blob, projectId, filename);
              if (stored.objectUrl) {
                setTimeout(() => {
                  triggerDownload(stored.objectUrl!, filename);
                }, 500);
              }
            } else if (downloadUrl) {
  ...
  generateOutlineStream: async (lockPageCount?: boolean) => {
    ...
    const streamState: {
      doneData?: { total: number; pages: any[]; complete?: boolean };
      errorMessage?: string;
    } = {};
    ...
    try {
      await api.generateOutlineStream(currentProject.id!, {
        ...
        onDone: (data) => { streamState.doneData = data; },
        onError: (message) => {
          console.error('[流式大纲] 错误:', message);
          streamState.errorMessage = message || t('store.generateOutlineFailed');

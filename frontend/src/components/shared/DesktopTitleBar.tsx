...
type DesktopDragStyle = CSSProperties & {
...
export function DesktopTitleBar() {
  ...
  const titleBarStyle: DesktopDragStyle = {
    position: 'fixed',
    top: 0,
    left: 0,
    right: 0,
    ...
    backgroundColor: isDark ? '#15172B' : '#ffffff',
    borderBottom: isDark ? '1px solid rgba(255, 255, 255, 0.10)' : '1px solid #E2E5F5',
    paddingLeft: isMac ? macLeadingSpace : 0,
  };
  const noDragStyle: DesktopDragStyle = {
    display: 'flex',
    alignItems: 'center',
    gap: 2,
    paddingRight: 4,
  ...
  const windowControlsStyle: DesktopDragStyle = {
    display: 'flex',
    alignItems: 'center',
    height: '100%',
    WebkitAppRegion: 'no-drag',
  ...
  return (
    ...
    >
      ...
      >
        <BrandMark className="h-[18px] w-[18px] rounded-md" />
        <span style={{ fontSize: 13, fontWeight: 600, lineHeight: '18px', color: isDark ? '#F4F5FF' : '#24264F', letterSpacing: 0 }}>
          StableAPI幻灯片
        </span>
      </div>
            ...
            >
              {([['light', Sun, t('themeLight')], ['dark', Moon, t('themeDark')], ['system', Monitor, t('themeSystem')]] as const).map(([value, Icon, label]) => (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-gray-100 dark:hover:bg-zinc-700 ${theme === value ? 'text-[#7B83FF]' : 'text-gray-700 dark:text-zinc-300'}`}
                >
                  <Icon size={12} />
                  <span>{label}</span>
                </button>

...
import { Sparkles, FileText, Palette, MessageSquare, Download, ChevronLeft, ChevronRight, ExternalLink, Settings, Check, Video, Info } from 'lucide-react';
...
const i18nDict = {
  zh: {
    guide: {
      ...
      gallery: '结果案例',
      gallerySub: '以下是使用 StableAPI幻灯片生成的 PPT 案例展示',
      galleryMore: '查看更多使用案例',
      hi: '欢迎使用 StableAPI幻灯片！',
      hiSub: '在开始前，让我们先完成基础配置',
      s1: '配置 API Key',
      s1d: '前往设置页面，配置项目需要使用的API服务，包括：',
      s1i: ['您的 AI 服务提供商的 API Base 和 API Key', '配置文本、图像生成模型和图像描述模型', '若需要文件解析功能，请配置 MinerU Token', '若需要可编辑导出功能，请配置 MinerU Token 和 Baidu API Key'],
      s2: '保存并测试',
      s2d: '配置完成后，务必点击「保存设置」按钮，然后在页面底部进行服务测试，确保各项服务正常工作。',
      s3: '开始创作',
      s3d: '配置成功后，返回首页即可开始使用 AI 生成精美的 PPT！',
      s4: '*问题反馈',
      s4d: '若使用过程中遇到问题，可通过部署方提供的反馈渠道联系支持。',
      issueLink: '问题反馈',
      settingsBtn: '前往设置页面',
      featuresOverview: '功能总览',
      featuresOverviewDesc: '了解 StableAPI幻灯片 的完整工作流程、创作路径、素材支持和导出能力。',
      ...
      feat: {
        ...
        vibe: { t: '「Vibe」式自然语言修改', d: '不再受限于复杂的菜单按钮，直接通过自然语言下达修改指令。', items: ['局部重绘：对不满意的区域进行口头式修改（如「把这个图换成饼图」）', '整页优化：基于已配置的图像模型生成高清、风格统一的页面'] },
  ...
  en: {
    guide: {
      ...
      gallery: 'Showcases',
      gallerySub: 'Here are PPT examples generated with StableAPI Slides',
      galleryMore: 'View more examples',
      hi: 'Welcome to StableAPI Slides!',
      ...
      s1i: ["Your AI service provider's API Base and API Key", 'Configure text, image generation, and image caption models', 'If you need file parsing, configure MinerU Token', 'If you need editable export, configure MinerU Token and Baidu API Key'],
      s2: 'Save and Test',
      ...
      s4: '*Feedback',
      s4d: 'If you encounter issues while using, contact support through the feedback channel provided by your deployment.',
      issueLink: 'Feedback',
      settingsBtn: 'Go to Settings',
      featuresOverview: 'Features Overview',
      featuresOverviewDesc: 'Learn the full workflow, creation paths, material support, and export options in StableAPI Slides.',
...
const FEATURES: { key: string; icon: React.ReactNode }[] = [
  { key: 'paths', icon: <Sparkles className="text-[#7B83FF]" size={24} /> },
  { key: 'parse', icon: <FileText className="text-[#45B7D1]" size={24} /> },
  { key: 'vibe', icon: <MessageSquare className="text-[#AD73F6]" size={24} /> },
  { key: 'export', icon: <Download className="text-[#7B83FF]" size={24} /> },
  { key: 'video', icon: <Video className="text-[#98A0FF]" size={24} /> },
];
...
const renderSetupPage: PageRenderer = ({ t, lang, navigate, onClose }) => {
  const steps = [
    ...
    { num: <Check size={18} />, bg: 'bg-[#7B83FF]', content: (
      <div className="flex-1 space-y-2">
  ...
  return (
    <div className="space-y-6">
        ...
        {steps.map((s, i) => (
          <div
            ...
            className={`flex gap-4 p-4 rounded-xl border ${
              s.highlight
                ? 'bg-gradient-to-r from-banana-50 dark:from-background-primary to-[#ECEFFF] border-banana-200'
      ...
      <div className="flex gap-4 p-4 bg-white dark:bg-background-secondary rounded-xl border border-gray-200 dark:border-border-primary">
        <div className="flex-shrink-0 w-8 h-8 bg-[#AD73F6] text-white rounded-full flex items-center justify-center font-bold">4</div>
        <div className="flex-1 space-y-2">
      ...
      <div className="flex gap-2 bg-[#F0EDFF] dark:bg-banana-900/30 border border-banana-200 dark:border-banana-700/40 rounded-lg p-3">
        <Info size={14} className="mt-0.5 flex-shrink-0 text-banana-700 dark:text-banana-300" />
        <p className="text-xs text-[#343667] dark:text-banana-200">
          <strong>{t('guide.hint')}</strong>：{t('guide.hintBody')}
        </p>
      </div>

      <div className="flex justify-center pt-2">
        <Button onClick={() => { onClose(); navigate('/settings', { state: { from: window.location.pathname } }); }} className="bg-gradient-to-r from-[#AD73F6] to-[#7B83FF] text-white shadow-lg" icon={<Settings size={18} />}>
          {t('guide.settingsBtn')}
        </Button>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <button onClick={prev} className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-all">
          <ChevronLeft size={20} />
        </button>
        <button onClick={next} className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-all">
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="" size="lg">
      <div className="space-y-6">
        ...
        <div className="text-center pb-4 border-b border-gray-100 dark:border-border-primary">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-banana-50 dark:from-background-primary to-[#ECEFFF] rounded-lg mb-3">
        ...
        <div className="pt-4 border-t flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            {pageIdx < PAGES.length - 1 ? (
              <Button onClick={() => setPageIdx(pageIdx + 1)} icon={<ChevronRight size={16} />} size="sm" className="bg-gradient-to-r from-[#AD73F6] to-[#7B83FF] text-white">
                {t('guide.next')}
              </Button>
            ) : (

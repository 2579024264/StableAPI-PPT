...
import { FileText, Upload, X, Loader2, CheckCircle2, XCircle, RefreshCw, ArrowUpDown, AlertTriangle, Info } from 'lucide-react';
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={t('referenceFile.title')} size="lg">
      <div className="space-y-4">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-shrink-0">
                        <div className="w-10 h-10 bg-[#ECEFFF] dark:bg-banana-900/30 rounded-lg flex items-center justify-center">
                          <FileText className="w-5 h-5 text-banana-600 dark:text-banana-300" />
                        </div>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                          ...
                          <p className="text-xs text-gray-600 dark:text-foreground-tertiary">
                            {getStatusText(file)}
                            {isPending && (
                              <span className="ml-1 text-banana-600 dark:text-banana-300">{t('referenceFile.parseOnConfirm')}</span>
                            )}
                          </p>
                        </div>
                        ...
                        {file.parse_status === 'completed' && 
                         ...
                         file.image_caption_failed_count > 0 && (
                          <p className="mt-1 flex items-center gap-1 text-xs text-banana-600 dark:text-banana-300">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-border-primary">
          <p className="flex items-start gap-1.5 text-xs text-gray-500 dark:text-foreground-tertiary">
            <Info size={13} className="mt-0.5 flex-shrink-0 text-banana-600 dark:text-banana-300" />
            <span>{t('referenceFile.autoParseHint')}</span>
          </p>
          <div className="flex items-center gap-2">

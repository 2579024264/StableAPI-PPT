...
const AiRefineInputComponent: React.FC<AiRefineInputProps> = ({
  ...
  return (
    <div className={isCompactMode ? `group ${className}` : `group bg-gradient-to-r from-[#F0EDFF] to-[#F1F6FF] rounded-lg p-3 md:p-4 border border-[#E2E5F5] ${className}`}>
      {/* 标题和历史按钮 - 仅非紧凑模式显示 */}
      {!isCompactMode && (
        <div className="flex items-center justify-between mb-2 md:mb-3">
          <div className="flex items-center gap-2">
            <Sparkles size={16} className="text-banana-600 md:w-[18px] md:h-[18px]" />
          ...
          {history.length > 0 && (
            <button
              ...
              className="flex items-center gap-1 text-xs text-banana-600 hover:text-banana-800 transition-colors"
            >
              <History size={14} />
      ...
      {showHistory && history.length > 0 && (
        <div className={`${isCompactMode ? 'mb-2' : 'mb-3'} p-2 bg-white dark:bg-background-secondary rounded border ${isCompactMode ? 'border-gray-200 dark:border-border-primary shadow-sm dark:shadow-background-primary/30' : 'bg-white/70 border-banana-100'} max-h-32 overflow-y-auto`}>
          ...
          <ul className="space-y-1">
            {history.map((req, idx) => (
              <li key={idx} className="text-xs text-gray-700 dark:text-foreground-secondary flex items-start gap-1">
                <span className="text-banana-400 flex-shrink-0">{idx + 1}.</span>
      ...
      <div className="flex gap-2 items-center relative">
        ...
        {isCompactMode && (
          <>
            <Sparkles size={16} className={`flex-shrink-0 transition-colors ${isSubmitting ? 'text-banana-500' : 'text-banana-600'}`} />
            {history.length > 0 && (
              <button
                onClick={() => setShowHistory(!showHistory)}
                className="flex items-center gap-1 text-xs text-gray-500 dark:text-foreground-tertiary hover:text-banana-600 transition-colors flex-shrink-0"
        ...
        <div className="flex-1 relative">
          <input
            ...
            className={`w-full px-3 py-1.5 text-sm border ${isCompactMode ? 'border-gray-200 dark:border-border-primary' : 'border-gray-300 dark:border-border-primary'} rounded-lg focus:outline-none focus:ring-2 focus:ring-banana-500 focus:border-transparent transition-all ${
              isSubmitting ? 'animate-gradient-x bg-gradient-to-r from-banana-100 via-[#ECEFFF] to-banana-100 bg-[length:200%_100%]' : 'bg-white dark:bg-background-secondary'
            }`}
        ...
        <button
          ...
          className={`flex-shrink-0 w-9 h-9 flex items-center justify-center rounded-lg transition-all ${
            !requirement.trim() || isSubmitting
              ...
              : 'bg-gradient-to-r from-[#AD73F6] to-[#7B83FF] text-white hover:brightness-105 active:translate-y-px'

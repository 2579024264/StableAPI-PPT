  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-background-secondary rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-border-primary bg-[#F0EDFF]">
          <div className="flex items-center gap-2">
            <AlertTriangle size={18} className="text-banana-600" />
            <h3 className="text-base font-semibold text-banana-800">
              {t('export.warningsCount', { count: warnings.length })}
            </h3>
          </div>
          <button
            onClick={onClose}
            className="p-1 hover:bg-banana-100 rounded transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="space-y-2">
            {warnings.map((warning, idx) => (
              <div
                key={idx}
                className="flex items-start gap-2 p-2 bg-[#F0EDFF] border border-banana-200 rounded text-sm text-[#343667]"
              >
                <span className="text-banana-500 mt-0.5">•</span>
        ...
        {task.status === 'FAILED' && task.errorMessage && (
          <div className="mt-2 space-y-2">
            ...
            {task.progress?.help_text && (
              <div className="p-2 bg-[#F0EDFF] dark:bg-banana-900/30 border border-banana-200 dark:border-banana-700/40 rounded">
                <div className="flex items-start gap-2">
                  <HelpCircle size={14} className="text-banana-600 flex-shrink-0 mt-0.5" />
                  <p className="text-xs text-banana-700 dark:text-banana-300">
                    {task.progress.help_text}
                  </p>
                </div>
              </div>
        ...
        {hasWarnings && (
          <>
            <button
              ...
              className="mt-1.5 w-full text-left px-2 py-1.5 bg-[#F0EDFF] border border-banana-200 rounded hover:bg-banana-100 transition-colors"
            >
              <div className="flex items-center gap-1.5">
                <AlertTriangle size={12} className="text-banana-600 flex-shrink-0" />
                <span className="text-xs font-medium text-banana-700">
                  {t('export.warnings', { count: task.progress?.warnings?.length ?? 0 })}
                </span>
                <span className="text-[11px] text-banana-500 ml-auto">
...
const FileTypeIcon: React.FC<{ type: string }> = ({ type }) => {
  switch (type) {
    case 'video': return <Film size={14} className="text-[#AD73F6]" />;
    case 'pptx': return <FileSpreadsheet size={14} className="text-[#7B83FF]" />;
    case 'pdf': return <FileText size={14} className="text-[#45B7D1]" />;
    case 'images': case 'image': return <Image size={14} className="text-[#5DD39E]" />;
